...
function MarketingCFeedb(props) {
  ...
  useEffect(() => {
    const token = localStorage.getItem("token");
    axios
      .post(`https://5d6b-14-231-219-38.ngrok-free.app/article/GetUpLoadedFiles?articleId=${id}`, null, {
        headers: {
          "ngrok-skip-browser-warning": true,
  ...
  const handleDownloadAllFiles = async () => {
    ...
    try {
      const response = await axios.post(
        `https://5d6b-14-231-219-38.ngrok-free.app/article/GetFile?articleId=${articleId}`,
        null,
        {
          headers: {
            "Content-Type": "application/json",
